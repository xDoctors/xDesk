...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/xdesk.exe')
        system2('mv target/release/xdesk.exe target/release/xDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\xdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/xDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
